...
export default function PlatformDocumentsClient({
  ...
  const [confirmDeleteTpl, setConfirmDeleteTpl] = useState<Template | null>(
    null,
  );
  ...
  async function handleFormSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!formData.rule_heading.trim()) {
      setFormError("Form description is required.");
      return;
    }
    setFormSaving(true);
    setFormError(null);
  ...
  async function handleTemplateSubmit(e: React.FormEvent) {
    ...
    if (!editingTemplate && !tplFile) {
      setTplError("Please select a file.");
      return;
    }
  ...
  const filteredForms = forms.filter((f) => {
    ...
    return (
      !q ||
      (f.form_no ?? "").toLowerCase().includes(q) ||
      f.rule_heading.toLowerCase().includes(q) ||
      (f.rule_no ?? "").toLowerCase().includes(q) ||
      fmtDate(f.created_at).toLowerCase().includes(q)
    );
  });
  ...
  const pagedForms = filteredForms.slice(
    (frmPageSafe - 1) * frmPerPage,
    frmPageSafe * frmPerPage,
  );
  ...
  const pagedTemplates = filteredTemplates.slice(
    (tplPageSafe - 1) * tplPerPage,
    tplPageSafe * tplPerPage,
  );
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-secondary text-sm mt-0.5">
          Platform-level forms reference and templates library
        </p>
      </div>
        ...
        <div className="flex gap-1 bg-surface-hover p-1 rounded-lg">
          {tabs.map((tab) => (
            ...
            >
              {tab.label}
              <span
                className={`ml-2 text-xs px-1.5 py-0.5 rounded-full ${activeTab === tab.key ? "bg-white/20 text-white" : "bg-white text-secondary"}`}
              >
                {tab.count}
              </span>
            </button>
        ...
        <div className="space-y-3">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-table-header border-b-2 border-table-header-border">
                    ...
                    <th className="text-center text-nowrap px-4 py-3 font-semibold text-heading w-20">
                      Form No.
                    </th>
                    ...
                    <th className="text-center px-4 py-3 font-semibold text-heading w-20">
                      Section
                    </th>
                    <th className="text-center px-4 py-3 font-semibold text-heading w-28 whitespace-nowrap">
                      Uploaded on
                    </th>
                    <th className="text-center px-4 py-3 font-semibold text-heading w-16">
                      Files
                    </th>
                    {canEdit && (
                      <th className="text-center px-4 py-3 font-semibold text-heading w-24">
                        Actions
                      </th>
                    )}
                  </tr>
                  ...
                  {pagedForms.length === 0 ? (
                    ...
                    pagedForms.map((f, i) => (
                      <tr
                        key={f.id}
                        onClick={() =>
                          f.url ? window.open(f.url, "_blank") : undefined
                        }
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-center text-nowrap text-secondary">
                          {f.form_no || "—"}
                        </td>
                        <td
                          className="px-4 py-3 text-secondary"
                          title={f.rule_heading}
                        >
                          <span
                            className={
                              f.url ? "text-accent hover:underline" : ""
                            }
                          >
                            {f.rule_heading}
                          </span>
                        ...
                        <td className="px-4 py-3 text-center text-secondary whitespace-nowrap">
                        ...
                        {canEdit && (
                          ...
                          >
                            <div className="flex items-center justify-center gap-0.5">
                              ...
                              <button
                                onClick={() => setConfirmDeleteForm(f)}
                                title="Delete row"
          ...
          <div className="mt-1 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3 text-sm text-secondary">
              <span>
                Showing {frmFrom}–{frmTo} of {frmTotal} form
                {frmTotal !== 1 ? "s" : ""}
              </span>
            ...
            {frmTotalPages > 1 && (
              <div className="flex items-center gap-1">
                <button
                  onClick={() => setFrmPage((p) => Math.max(1, p - 1))}
                  disabled={frmPageSafe === 1}
                ...
                {pageNums(frmPageSafe, frmTotalPages).map((p, i) =>
                  p === "..." ? (
                    ...
                    <button
                      key={p}
                      onClick={() => setFrmPage(p as number)}
                      className={btnPage(p === frmPageSafe)}
                    >
                      {p}
                    </button>
                  ),
                ...
                <button
                  onClick={() =>
                    setFrmPage((p) => Math.min(frmTotalPages, p + 1))
                  }
                  disabled={frmPageSafe === frmTotalPages}
                  className="h-9 px-3 text-sm border border-border rounded-lg text-heading hover:bg-page disabled:opacity-40 disabled:cursor-not-allowed transition"
                >
                  Next →
                </button>
        ...
        <div className="space-y-3">
          ...
          {pagedTemplates.length === 0 ? (
            <div className="bg-white border border-border rounded-xl p-16 text-center">
              <svg
                className="w-10 h-10 text-border-strong mx-auto mb-3"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={1.5}
                ...
                <tbody>
                  {pagedTemplates.map((t, i) => (
                    ...
                    >
                      ...
                      <td
                        className="px-4 py-3 text-secondary max-w-105 truncate"
                        title={t.description ?? ""}
                      >
                        {t.description ?? "—"}
                      </td>
          ...
          <div className="mt-1 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3 text-sm text-secondary">
              <span>
                Showing {tplFrom}–{tplTo} of {tplTotal} template
                {tplTotal !== 1 ? "s" : ""}
              </span>
              <div className="flex items-center gap-1.5">
                <span className="text-xs">Show</span>
              ...
              <div className="flex items-center gap-1">
                ...
                {pageNums(tplPageSafe, tplTotalPages).map((p, i) =>
                  p === "..." ? (
                    ...
                    <button
                      key={p}
                      onClick={() => setTplPage(p as number)}
                      className={btnPage(p === tplPageSafe)}
                    >
                      {p}
                    </button>
                  ),
                ...
                <button
                  onClick={() =>
                    setTplPage((p) => Math.min(tplTotalPages, p + 1))
                  }
                  disabled={tplPageSafe === tplTotalPages}
                  className="h-9 px-3 text-sm border border-border rounded-lg text-heading hover:bg-page disabled:opacity-40 disabled:cursor-not-allowed transition"
                >
                  Next →
                </button>
            ...
            <form onSubmit={handleFormSubmit} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData((p) => ({ ...p, rule_no: e.target.value }))
                    }
                    className={inp}
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData((p) => ({ ...p, form_no: e.target.value }))
                    }
                    className={inp}
                  />
                </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData((p) => ({ ...p, rule_heading: e.target.value }))
                  }
                  className={inp}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData((p) => ({ ...p, page_no: e.target.value }))
                  }
                  className={inp}
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-secondary mb-1">
                  URL{" "}
                  <span className="text-muted">(link to form document)</span>
                </label>
                <input
                  type="url"
                  value={formData.url ?? ""}
      ...
      {showTemplateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 mx-4">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-base font-semibold text-heading">
                {editingTemplate ? "Edit Template" : "Upload Template"}
              </h2>
              <button
              ...
              {!editingTemplate ? (
                ...
                <div>
                  ...
                  <p className="text-xs text-muted mt-1">
                    To replace the file, delete this template and upload a new
                    one.
                  </p>
                </div>
              )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl border border-border p-6 w-full max-w-sm mx-4">
            ...
            <p className="text-sm text-secondary mb-5">
              &quot;{confirmDeleteForm.rule_heading}&quot; will be permanently
              removed.
            </p>
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={deletingFormId === confirmDeleteForm.id}
              ...
              >
                {deletingFormId === confirmDeleteForm.id
                  ? "Deleting…"
                  : "Delete"}
              </button>
            </div>
      ...
      {confirmDeleteTpl && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl border border-border p-6 w-full max-w-sm mx-4">
            ...
            <p className="text-sm text-secondary mb-5">
              &quot;{confirmDeleteTpl.name}&quot; will be permanently removed.
            </p>
            <div className="flex gap-3">
              <button
              ...
              <button
                onClick={() => handleDeleteTemplate(confirmDeleteTpl)}
                disabled={deletingTplId === confirmDeleteTpl.id}
                className="flex-1 px-4 py-2 text-sm bg-red-600 hover:bg-red-700 text-white rounded-lg font-medium transition disabled:opacity-60"
              >
