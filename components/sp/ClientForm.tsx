...
export default function ClientForm({ mode, clientId, initialData, initialCompliance, readOnly = false, businessTypes }: Props) {
  ...
  const fieldClass = `w-full px-3 py-2 text-sm border border-[#E5E7EB] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1E3A5F] ${readOnly ? "bg-[#F8F9FA] text-[#6B7280] cursor-not-allowed" : ""}`;
  const inp = `w-full px-2.5 py-1.5 text-sm border border-[#E5E7EB] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1E3A5F] ${readOnly ? "bg-[#F8F9FA] text-[#6B7280] cursor-not-allowed" : ""}`;
            ...
            <tbody className="divide-y divide-[#E5E7EB]">
                ...
                <tr key={key} className="hover:bg-[#FAFAFA]">
                  ...
                  <td className="px-4 py-3">
                    <div className="relative">
                      <input type={compliance[key].showCredential ? "text" : "password"} value={compliance[key].credential} onChange={(e) => updateCompliance(key, "credential", e.target.value)} disabled={readOnly} placeholder="Password" className={`${inp} pr-8`} />
              ...
              {extraRows.map((row) => (
                <tr key={row.rowId} className="hover:bg-[#FAFAFA]">
                  ...
                  <td className="px-4 py-3">
                    <div className="relative">
                      <input type={row.showCredential ? "text" : "password"} value={row.credential} onChange={(e) => updateExtraRow(row.rowId, "credential", e.target.value)} disabled={readOnly} placeholder="Password" className={`${inp} pr-8`} />
