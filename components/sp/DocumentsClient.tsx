...
export default function DocumentsClient({ documents, forms, templates, canEdit }: Props) {
  const router = useRouter();
  const [activeTab, setActiveTab] = useState<"documents" | "forms" | "templates">("documents");
  ...
  const tabs = [
    { key: "documents" as const, label: "Appeal Documents", count: documents.length },
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        ...
        <p className="text-[#6B7280] text-sm mt-0.5">Appeal documents, forms reference and templates library</p>
      </div>
      ...
      {activeTab === "documents" && (
        documents.length === 0 ? (
          <div className="bg-white border border-[#E5E7EB] rounded-xl p-16 text-center">
            ...
            <p className="text-[#9CA3AF] text-xs mt-1">Upload documents from within an appeal's detail page.</p>
          </div>
        ) : (
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {documents.map((doc) => {
                    ...
                    const appealLabel = [appeal?.financial_year, appeal?.assessment_year].filter(Boolean).join(" / ") || appeal?.act_regulation || "—";
                    ...
                    return (
                      <tr key={doc.id} className="border-b border-[#E5E7EB] last:border-0 hover:bg-[#F8F9FA] transition-colors">
                        ...
                        <td className="px-4 py-3 text-[#6B7280] whitespace-nowrap truncate max-w-[160px]">{appeal?.client_org?.name ?? "—"}</td>
                        <td className="px-4 py-3 text-[#6B7280] whitespace-nowrap">{uploader ? `${uploader.first_name} ${uploader.last_name}` : "—"}</td>
                        <td className="px-4 py-3 text-[#6B7280] whitespace-nowrap">{fmtDate(doc.created_at)}</td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            <a href={doc.file_url} target="_blank" rel="noopener noreferrer" className="text-xs font-medium text-[#4A6FA5] hover:text-[#1E3A5F]">View</a>
                            {appeal?.id && (
                              <Link href={`/appeals/${appeal.id}`} className="text-xs font-medium text-[#6B7280] hover:text-[#1A1A2E]">Appeal →</Link>
                            )}
                          </div>
                        </td>
                      </tr>
